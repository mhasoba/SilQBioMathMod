...
##store results
results<-matrix(NA,nrow=length(times)+1,ncol=9)
...
for (i in (1:length(times))) {
	...
	E[1,1]<- E[1,1]+mu[1]*((c.conv[1,1]*(kcat[1,1]*S[1]/(Km[1,1]+S[1])))-(c.conv[2,1]*(kcat[2,1]*S[2]/(Km[2,1]+S[2]))))*time.step
	E[1,2]<- E[1,2]+mu[2]*((c.conv[1,2]*(kcat[1,2]*S[1]/(Km[1,2]+S[1])))-(c.conv[2,2]*(kcat[2,2]*S[2]/(Km[2,2]+S[2]))))*time.step
...
matplot(results[,1], results[,6:9], type="l", xlab="Time", 
        ylab="Density/Concentration",col=c("black","red"),lty=c(1,1,3,3))
legend(x=100,y=0.5,legend=c("Spec1.Subst1","Spec1.Subst2","Spec2.subst1","Spec2.subst2"), col=c("black","red"),lty=c(1,1,3,3))
